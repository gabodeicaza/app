...
def _project_public(p: dict) -> dict:
    ...
    p.setdefault("template_map", None)
    return p
...
_TEMPLATE_KIND_EXT = {
    ...
    "docx": ".docx",
    "pptx": ".pptx",
    # Imagen de mapa/ubicación institucional (para PPTX narrativo).
    # Se preserva la extensión original del archivo subido.
    "map": ".img",
}
_TEMPLATE_KIND_MIME = {
    "pdf": {"application/pdf"},
...
def _template_path_for(pid: str, kind: str, ext_override: Optional[str] = None) -> Path:
    ext = ext_override or _TEMPLATE_KIND_EXT[kind]
...
async def upload_project_template(
    ...
    """Sube una plantilla PDF/DOCX/PPTX/MAP para un proyecto.
    - pdf/docx/pptx: se usan como fondo/base para exportaciones.
    - map: imagen (jpg/png/webp) que se incrustará en la slide 'Mapa/Ubicación' del PPTX
      narrativo, en la shape llamada 'MAPA' (o como fondo si no existe).
    Solo Coordinador General o Jefe de Proyecto.
    """
    kind = (kind or "").lower()
    if kind not in _TEMPLATE_KIND_EXT:
        raise HTTPException(400, "kind debe ser 'pdf', 'docx', 'pptx' o 'map'")
    ...
    # Para "map", preservar la extensión original (jpg/png/webp)
    if kind == "map":
        orig_name = (file.filename or "map.jpg").lower()
        ext = ".jpg"
        for cand in (".jpg", ".jpeg", ".png", ".webp"):
            if orig_name.endswith(cand):
                ext = cand
                break
        # Eliminar cualquier plantilla map previa (extensión distinta) para evitar residuos
        for cand_ext in (".jpg", ".jpeg", ".png", ".webp", ".img"):
            try:
                _template_path_for(pid, "map", cand_ext).unlink(missing_ok=True)
            except Exception:
                pass
        dest_path = _template_path_for(pid, "map", ext)
    else:
...
async def export_reports_pptx(
    ...
    # === Helpers de manipulación PPTX (modo narrativo) ===
    _REL_NS = "{http://schemas.openxmlformats.org/officeDocument/2006/relationships}"

    def _pptx_replace_tokens(slide, tokens: dict):
        """Reemplaza {{TOKEN}} en todos los text frames del slide.
        Preserva el formato del primer run del párrafo donde aparece el token.
        Los tokens no encontrados se dejan intactos (ayuda al debug en la plantilla).
        """
        for shape in slide.shapes:
            ...
            for para in shape.text_frame.paragraphs:
                ...
                for tk, val in tokens.items():
                    if val is None:
                        val = ""
                    new_text = new_text.replace("{{" + tk + "}}", str(val))
                if new_text == para_text:
                    continue
                # Consolida en el primer run manteniendo formato
                runs[0].text = new_text
                for r in runs[1:]:
                    r.text = ""

    def _pptx_replace_image_by_shape_name(slide, name: str, image_bytes: bytes) -> bool:
        """Sustituye la shape 'name' (case-insensitive) por un picture con las mismas
        posiciones/tamaño. Sirve tanto para placeholders visuales (rectángulos) como
        para pictures ya existentes. Devuelve True si se realizó reemplazo."""
        target = None
        for sh in list(slide.shapes):
            if (sh.name or "").strip().upper() == name.upper():
                target = sh
                break
        if target is None:
            return False
    ...
    def _pptx_has_shape(slide, name: str) -> bool:
        for sh in slide.shapes:
            if (sh.name or "").strip().upper() == name.upper():
                return True
        return False

    def _pptx_duplicate_slide(prs, source_slide):
        """Clona un slide preservando shapes, texto e imágenes (rels remapeados).
        Se agrega al FINAL de la presentación."""
        blank_layout = source_slide.slide_layout
        dest = prs.slides.add_slide(blank_layout)
        # Limpia cualquier shape auto-creado por el layout
        for shp in list(dest.shapes):
            try:
                shp._element.getparent().remove(shp._element)
            except Exception:
                pass
        # Registra rels del origen y remapea rIds
        old_to_new = {}
        for rel_id, rel in source_slide.part.rels.items():
            if "notesSlide" in rel.reltype:
                continue
            try:
                new_rel = dest.part.rels.get_or_add(rel.reltype, rel.target_part)
                old_to_new[rel_id] = new_rel.rId
            except Exception:
                pass
        # Copia shapes en profundidad y remapea r:embed/r:link
    ...
    def _report_tokens_for(base_tokens: dict, node: dict, report: dict) -> dict:
        """Genera el diccionario de tokens {{...}} para un slide de Nodo/Reporte."""
        node_path = path_cache.get(node["id"]) or node.get("name", "")
        ...
        primera_str = _raw_to_str(report.get("primera_lectura"))
        ultima_str = _raw_to_str(report.get("ultima_lectura"))
        _av = report.get("avance")
        avance_str = str(_av).strip() if _av not in (None, "") else "—"
        ts = report.get("created_at")
        fecha_str = _fmt_fecha_dd_mm_yyyy(ts) if isinstance(ts, datetime) else "—"
        nombre = report.get("captured_by_name") or "—"
        unidad_r = (report.get("unidad") or "m").strip() or "m"
        personal_list = [p for p in (report.get("personnel") or []) if p]
        equipo_list = [e for e in (report.get("equipment") or []) if e]
        personal_str = ", ".join(personal_list) if personal_list else "N/A"
        equipo_str = ", ".join(equipo_list) if equipo_list else "N/A"
        obs_str = (report.get("observaciones") or report.get("notes") or "").strip() or "N/A"
        incidencias_str = (report.get("incidencias") or "").strip() or "Sin incidencias."
        sev = (report.get("severidad") or "informativo").lower()
        sev_label = _severidad_label(sev).upper()
        try:
            coord_text = _format_measurement_for_display(report) or "—"
        except Exception:
            coord_text = "—"

        # Actividades combinadas: observaciones + métricas
        _act_lines = []
        if obs_str and obs_str != "N/A":
        ...
        actividades_str = " ".join(_act_lines)
        ...
        rt.update({
            ...
            "SITUACION_SOCIAL": incidencias_str,
            "INCIDENCIAS": incidencias_str,
            "ACTIVIDADES": actividades_str,
            "OBSERVACIONES": obs_str,
            "PERSONAL": personal_str,
            "EQUIPO": equipo_str,
            "PRIMERA_LECTURA": f"{primera_str} {unidad_r}",
            "ULTIMA_LECTURA": f"{ultima_str} {unidad_r}",
            "AVANCE": avance_str,
            "UNIDAD": unidad_r,
            "SEVERIDAD": sev_label,
        })
    ...
    def _build_pptx_narrative(prs, tpl_map_path: Optional[str]) -> bytes:
        """Construye el PPTX en modo narrativo por nodo, usando la plantilla del usuario
        como índice de slides. NO agrega elementos visuales nuevos: sólo reemplaza tokens
        y carga imágenes en shapes FOTO_1 / FOTO_2 / MAPA."""
        import base64 as _b64

        # Base de tokens (variables globales del proyecto/reporte)
        base_tokens = {
            "TITULO": project_name,
            "PROYECTO": project_name,
            "NOMBRE_PROYECTO": project_name,
            "CONTRATISTA": project_constructora,
            "CONSTRUCTORA": project_constructora,
            "CONTRATO": project_contract,
            "NO_CONTRATO": project_contract,
            "CLIENTE": project_cliente or "—",
            "OBJETO": project_objeto or "",
            "OBJETO_CONTRATO": project_objeto or "",
            "PERIODO": fechas_label,
            "FECHA_EXPORT": _fmt_fecha_dd_mm_yyyy_hhmm(datetime.now(timezone.utc)),
            "AREA": area_label,
            "TOTAL_REPORTES": str(sum(len(v) for v in reports_by_node.values())),
        }

        # === Portada (slide 0) — reemplaza tokens en el lugar ===
        cover_slide = prs.slides[0]
        _pptx_replace_tokens(cover_slide, base_tokens)

        # === Mapa (slide 1) — tokens + imagen en shape 'MAPA' ===
        map_slide = prs.slides[1]
        _pptx_replace_tokens(map_slide, base_tokens)
        if tpl_map_path:
            try:
                with open(tpl_map_path, "rb") as f:
                    map_bytes = f.read()
                # Intenta shape "MAPA", si no existe prueba "UBICACION"/"MAP"
                replaced = _pptx_replace_image_by_shape_name(map_slide, "MAPA", map_bytes)
                if not replaced:
                    replaced = _pptx_replace_image_by_shape_name(map_slide, "UBICACION", map_bytes)
                if not replaced:
                    replaced = _pptx_replace_image_by_shape_name(map_slide, "MAP", map_bytes)
            except Exception:
                pass

        # === Referencias a los slides plantilla (índice 2 = Nodo, índice 3 = Galería opcional) ===
        # IMPORTANTE: clonar ANTES de mutar la plantilla original para preservar el diseño
        # entre iteraciones. Guardamos las shapes originales en un slide "sombra" en memoria.
        original_node_tpl_slide = prs.slides[2]
        original_gallery_tpl_slide = prs.slides[3] if len(prs.slides) >= 4 else None

        # Snapshot: clonamos el nodo-tpl al final y lo usaremos como source para clones adicionales
        # (así el primer nodo se pinta sobre la slide 2 original, y los siguientes sobre clones
        # basados en el snapshot congelado).
        # Recolección de pares (nodo, reporte) preservando orden institucional
        node_report_pairs = []
        for n in leaf_nodes:
            for r in (reports_by_node.get(n["id"]) or []):
                node_report_pairs.append((n, r))

        if not node_report_pairs:
            # No hay reportes: aún así reemplazamos tokens en el nodo tpl con mensaje amigable
            empty_tokens = dict(base_tokens)
            empty_tokens.update({
                "NODO": "Sin reportes en el período",
                "TRAMO": "—",
                "UBICACION": "—",
                "COORDENADAS": "—",
                "FECHA": "—",
                ...
                "SITUACION_SOCIAL": "—",
                "INCIDENCIAS": "—",
                "ACTIVIDADES": "No se registraron reportes durante el período seleccionado.",
                "OBSERVACIONES": "—",
                "PERSONAL": "—",
                "EQUIPO": "—",
                "PRIMERA_LECTURA": "—",
            ...
            _pptx_replace_tokens(original_node_tpl_slide, empty_tokens)
            _buf = io.BytesIO()
            prs.save(_buf)
            return _buf.getvalue()

        # ── Congela el diseño del template de Nodo antes de la primera mutación
        #    Clonamos al final y usamos esa clonación como source para los siguientes.
        node_source_snapshot = _pptx_duplicate_slide(prs, original_node_tpl_slide)
        gallery_source_snapshot = None
        if original_gallery_tpl_slide is not None:
            gallery_source_snapshot = _pptx_duplicate_slide(prs, original_gallery_tpl_slide)

        # Después de haber snapshoteado los templates, eliminamos las copias
        # de la lista visible arrastrándolas hasta el final. Sin embargo NO borramos
        # nada: sólo las usamos como fuentes. Marcadas para permanecer al final "ocultas"
        # visualmente. En la práctica se ven al final del PPTX pero con los tokens
        # sin reemplazar; el usuario puede eliminarlas si desea. Para evitar esto,
        # las hacemos pasar por el reemplazo de tokens con un valor neutro.
        _pptx_replace_tokens(node_source_snapshot, {k: "" for k in [
            "NODO", "TRAMO", "UBICACION", "COORDENADAS", "FECHA", "FECHA_REPORTE",
            "ESPECIALISTA", "CAPTURADO_POR", "SITUACION_SOCIAL", "INCIDENCIAS",
            "ACTIVIDADES", "OBSERVACIONES", "PERSONAL", "EQUIPO",
            "PRIMERA_LECTURA", "ULTIMA_LECTURA", "AVANCE", "UNIDAD", "SEVERIDAD",
        ]})
        if gallery_source_snapshot is not None:
            _pptx_replace_tokens(gallery_source_snapshot, base_tokens)

        # Ahora la lista actual: [Portada, Mapa, NodoOrig(vacío), (GaleríaOrig(vacía)?), NodoSnap, GallerySnap?]
        # Estrategia final: usar NodoOrig como slot para reporte #1, GaleríaOrig como slot
        # para galería #1_par1. Para nodos/galerías subsecuentes, clonamos node_source_snapshot
        # / gallery_source_snapshot y los agregamos al final.
        # Finalmente eliminamos las últimas 2 copias "snapshot" del sldIdLst para
        # que no aparezcan como slides vacíos al final.

        # Ordenación importante: primero llenar Nodo #1 y su Galería #1,
        # luego para cada nodo adicional clonar y llenar en orden nodo→galerías.
        # Como add_slide siempre agrega al final, el orden se mantiene naturalmente.

        # Placeholder: los slides clonados adicionales de nodo van antes que los de galería?
        # Con el flujo actual:
        #   Slides originales: 0=Portada, 1=Mapa, 2=NodoOrig, 3=GaleríaOrig, 4=NodoSnap, 5=GallerySnap
        # Al llenar reporte #1: usamos NodoOrig(2), GaleríaOrig(3) para su primer par.
        # Reporte #1 galerías pares extras: clonamos GallerySnap -> agrega al final (posición 6, 7...).
        #   Order: 0 Portada, 1 Mapa, 2 Nodo1, 3 Gal1par1, 4 NodoSnap(vacío), 5 GallerySnap(vacío),
        #          6 Gal1par2, 7 Gal1par3, ...
        # Los NodoSnap/GallerySnap vacíos quedan interpuestos → mal UX.
        # Por eso: los eliminamos del sldIdLst al final del proceso.

        # Iteramos
        for idx, (n, r) in enumerate(node_report_pairs):
            tokens = _report_tokens_for(base_tokens, n, r)
            if idx == 0:
                node_slide = original_node_tpl_slide
            else:
                node_slide = _pptx_duplicate_slide(prs, node_source_snapshot)
            _pptx_replace_tokens(node_slide, tokens)

            imgs = r.get("images") or []
            # Foto 1 en FOTO_1
            if imgs and _pptx_has_shape(node_slide, "FOTO_1"):
                try:
                    raw = _b64.b64decode(_strip_b64_prefix(imgs[0]))
                    _pptx_replace_image_by_shape_name(node_slide, "FOTO_1", raw)
                except Exception:
                    pass
            # Foto 2 en FOTO_2 (si existe en el template y hay al menos 2 fotos)
            if len(imgs) >= 2 and _pptx_has_shape(node_slide, "FOTO_2"):
                try:
                    raw = _b64.b64decode(_strip_b64_prefix(imgs[1]))
                    _pptx_replace_image_by_shape_name(node_slide, "FOTO_2", raw)
                except Exception:
                    pass

            # === Fotos extra (3ra en adelante) → slides de galería ===
            # Si el nodo template usa FOTO_1+FOTO_2, las extras empiezan en índice 2.
            # Si el nodo template sólo usa FOTO_1, las extras empiezan en índice 1.
            has_foto2 = _pptx_has_shape(node_slide, "FOTO_2")
            start_extra = 2 if has_foto2 else 1
            extras = imgs[start_extra:] if len(imgs) > start_extra else []
            gallery_source = gallery_source_snapshot or node_source_snapshot
            first_gallery_used = False
            for pair_idx in range(0, len(extras), 2):
                pair = extras[pair_idx:pair_idx + 2]
                if idx == 0 and not first_gallery_used and original_gallery_tpl_slide is not None:
                    gal_slide = original_gallery_tpl_slide
                    first_gallery_used = True
                else:
                    gal_slide = _pptx_duplicate_slide(prs, gallery_source)
                _pptx_replace_tokens(gal_slide, tokens)
                if len(pair) >= 1 and _pptx_has_shape(gal_slide, "FOTO_1"):
                    try:
                        raw = _b64.b64decode(_strip_b64_prefix(pair[0]))
                        _pptx_replace_image_by_shape_name(gal_slide, "FOTO_1", raw)
                    except Exception:
                        pass
                if len(pair) >= 2 and _pptx_has_shape(gal_slide, "FOTO_2"):
                    try:
                        raw = _b64.b64decode(_strip_b64_prefix(pair[1]))
                        _pptx_replace_image_by_shape_name(gal_slide, "FOTO_2", raw)
                    except Exception:
                        pass

        # === Cleanup: eliminar los snapshots "sombra" del sldIdLst ===
        # No borramos las parts (relaciones) para no romper referencias;
        # sólo removemos los IDs del listado visible para que no aparezcan
        # como slides vacíos al final del PPTX.
        try:
            sld_id_lst = prs.slides._sldIdLst
            entries = list(sld_id_lst)
            # Identificamos por rId hacia los parts snapshot
            targets_to_remove = set()
            snapshots = [node_source_snapshot]
            if gallery_source_snapshot is not None:
                snapshots.append(gallery_source_snapshot)
            snap_parts = {id(s.part) for s in snapshots}
            for entry in entries:
                r_id = entry.get(_REL_NS + "id")
                if not r_id:
                    continue
                try:
                    ...
                    continue
                if id(part) in snap_parts:
                    targets_to_remove.add(entry)
            for entry in targets_to_remove:
